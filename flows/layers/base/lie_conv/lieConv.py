...
from mixed_lipschitz import InducedNormLinear
import ipdb
...
def InducedNormLinearBNact(chin,chout,act='swish',bn=True):
    ...
    return nn.Sequential(
        Pass(InducedNormLinear(chin,chout),dim=1),
...
def InducedNormWeightNet(in_dim,out_dim,act,bn,k=32):
    return nn.Sequential(
        *InducedNormLinearBNact(in_dim, k, act, bn),
        *InducedNormLinearBNact(k, k, act, bn),
        *InducedNormLinearBNact(k, out_dim, act, bn))
...
class InducedNormPointConv(nn.Module):
    def __init__(self,chin,chout,mc_samples=32,xyz_dim=3,ds_frac=1,knn_channels=None,act='swish',bn=False,mean=False):
        super().__init__()
        self.chin = chin # input channels
        self.cmco_ci = 16 # a hyperparameter controlling size and bottleneck compute cost of weightnet
        ...
        self.knn_channels = knn_channels # number of xyz dims on which to compute knn
        self.mc_samples = mc_samples # number of samples to use to estimate convolution
        self.weightnet = InducedNormWeightNet(xyz_dim, self.cmco_ci, act, bn) # MLP - final layer to compute kernel vals (see A1)
        self.linear = InducedNormLinear(self.cmco_ci * chin, chout)        # final linear layer to compute kernel vals (see A1)
        self.mean=mean  # Whether or not to divide by the number of mc_samples
        assert ds_frac==1, "test branch no ds, will need to carefully check that subsample respects padding"
        self.subsample = FarthestSubsample(ds_frac,knn_channels=knn_channels) # Module for subsampling if ds_frac<1
...
class InducedNormLieConv(InducedNormPointConv):
...
@export
class ToyLieResnet(nn.Module,metaclass=Named):
    """ Generic LieConv architecture from Fig 5. Relevant Arguments:
        [Fill] specifies the fraction of the input which is included in local neighborhood.
                (can be array to specify a different value for each layer)
        [nbhd] number of samples to use for Monte Carlo estimation (p)
        ...
        [bn] whether or not to use batch normalization. Recommended in all cases except dynamical systems.
        """
    def __init__(self, chin=1, ds_frac=1, num_outputs=1, k=1536, nbhd=np.inf,
                act="swish", bn=True, num_layers=1, mean=True, per_point=True,pool=True,
                liftsamples=1, fill=1/4, group=SE3,knn=False,cache=False, **kwargs):
        super().__init__()
        if isinstance(fill,(float,int)):
            fill = [fill]*num_layers
        ...
        # self.conv = lambda ki,ko,fill: LieConv(ki, ko, mc_samples=nbhd, ds_frac=ds_frac, bn=bn,act=act, mean=mean,
                                # group=group,fill=fill,cache=cache,knn=knn,**kwargs)
        self.conv = LieConv(1, 1, mc_samples=nbhd, ds_frac=ds_frac, bn=bn,act=act, mean=mean,
                                group=group,fill=fill[0],cache=cache,knn=knn)
        self.conv2 = LieConv(1, 1, mc_samples=nbhd, ds_frac=ds_frac, bn=bn,act=act, mean=mean,
                                group=group,fill=fill[0],cache=cache,knn=knn)
        self.liftsamples = liftsamples
        self.per_point=per_point
        self.group = group
    ...
    def forward(self, x, coord_transform=None):
        ...
        with torch.no_grad():
            if self.lifted_coords is None:
                ...
                lifted_vals,lifted_mask = self.group.expand_like(values,mask,self.lifted_coords)
        ipdb.set_trace()
        out_coord, out_val, out_mask = self.conv((self.lifted_coords,lifted_vals,lifted_mask))
        out_coord_, out_val_, out_mask_ = self.conv2((out_coord,out_val,out_mask))
        return out
