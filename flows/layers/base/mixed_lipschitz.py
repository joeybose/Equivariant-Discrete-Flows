...
class InducedNormEquivarConv2d(nn.Module):
    ...
    def __init__(
        self, in_type, out_type, group_action_type, kernel_size, stride, padding, bias=True, coeff=0.97, domain=2, codomain=2,
        n_iterations=None, atol=None, rtol=None, **unused_kwargs
    ):
        del unused_kwargs
        ...
        self.atol = atol
        self.rtol = rtol
        self.equivar_conv = enn.R2Conv(in_type, out_type,
    ...
    def _initialize_u_v(self):
        self.weight, self.bias = self.equivar_conv.expand_parameters()
        with torch.no_grad():
            domain, codomain = self.compute_domain_codomain()
            if self.kernel_size == (1, 1):
                self.u.resize_(self.out_channels).normal_(0, 1)
    ...
    def compute_one_iter(self):
        ...
        if self.kernel_size == (1, 1):
            u = self.u.detach()
            v = self.v.detach()
            weight, expanded_bias = self.equivar_conv.expand_parameters()
    ...
    def forward(self, input):
        ...
        if is_this_a_tensor:
            ...
            output = F.conv2d(input.tensor, weight, expanded_bias, self.stride, self.padding, 1, 1)
...
if __name__ == '__main__':
    ...
    group_action_type = gspaces.FlipRot2dOnR2(N=4)
    ...
    m = InducedNormEquivarConv2d(in_type, out_type, group_action_type, 3, 1,
                                 1,domain=2, codomain=2,  atol=1e-3,
                                 rtol=1e-3)
    m2 = InducedNormConv2d(10, 2, 3, 1, 1, atol=1e-3, rtol=1e-3, domain=p, codomain=p)

    x = torch.randn(4, 1, 1, 2)
    x = enn.GeometricTensor(x, in_type)
    # W = m(x)
    # check_equivariance(group_action_type, in_type, x, m, 'GeomTensor')
    # check_equivariance(group_action_type, in_type, x.tensor, m, 'GeomTensor')
