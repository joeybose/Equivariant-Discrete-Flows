...
class MyR2Conv(EquivariantModule):
    ...
    def _make_params(self):
        ...
        u = w.new(height).normal_(0, 1)
        v = w.new(width).normal_(0, 1)
        u = l2normalize(u)
        v = l2normalize(v)
    ...
    def expand_parameters(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        while i < self.max_iters:
            _filter = self._update_u_v(_filter)
            if self.scale <=1:
                break
            # ipdb.set_trace()
            i+=1
        ...
        if i == self.max_iters:
            print("Lipschitz constant is %f" %(self.scale))
    ...
    def train(self, mode=True):
        ...
        return super(MyR2Conv, self).train(mode)
