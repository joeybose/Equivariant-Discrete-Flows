...
__all__ = ['iResBlock', 'Equivar_iResBlock']
    ...
    def _logdetgrad(self, x):
        ...
        with torch.enable_grad():
            if (self.brute_force or not self.training) and (x.squeeze().ndimension() == 2 and x.shape[-1] == 2):
                ...
                g = self.nnet(x)
                # Brute-force logdet only available for 2D.
                jac = geom_batch_jacobian(g.tensor, x)
                batch_dets = (jac[:, 0, 0] + 1) * (jac[:, 1, 1] + 1) - jac[:, 0, 1] * jac[:, 1, 0]
                return g, torch.log(torch.abs(batch_dets)).view(-1, 1)
            ...
            if not self.exact_trace:
                ...
                if torch.is_tensor(x):
                    ...
                    vareps = torch.randn_like(x.tensor)
                    # Do backprop-in-forward to save memory.
                    if self.training and self.grad_in_forward:
                        g, logdetgrad = mem_eff_wrapper(
                            estimator_fn, self.nnet, x.tensor, n_power_series, vareps, coeff_fn, self.training
                        )
                    else:
                        x = x.requires_grad_(True)
                        g = self.nnet(x)
                        logdetgrad = estimator_fn(g, x.tensor, n_power_series, vareps, coeff_fn, self.training)
            else:
                ############################################
                # Power series with exact trace computation.
                ############################################
