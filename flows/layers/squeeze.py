...
__all__ = ['SqueezeLayer', 'EquivariantSqueezeLayer']
...
class EquivariantSqueezeLayer(nn.Module):

    def __init__(self, downscale_factor):
        super(EquivariantSqueezeLayer, self).__init__()
        self.downscale_factor = downscale_factor

    def forward(self, x, logpx=None):
        squeeze_x = equivariant_squeeze(x, self.downscale_factor)
        if logpx is None:
            return squeeze_x
        else:
            return squeeze_x, logpx
    ...
    def inverse(self, y, logpy=None):
        unsqueeze_y = equivariant_unsqueeze(y, self.downscale_factor)
        if logpy is None:
            return unsqueeze_y
        else:
            return unsqueeze_y, logpy
...
def equivariant_squeeze(input, downscale_factor=2):
    '''
    [:, C, H*r, W*r] -> [:, C*r^2, H, W]
    '''
    in_type = input.type
